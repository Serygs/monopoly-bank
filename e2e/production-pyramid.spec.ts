...
test.describe('production browser pyramid', () => {
  ...
  test('six independent devices join by QR invitation and converge after reconnect', async ({
    ...
    const guests = await Promise.all(
      Array.from({ length: 5 }, (_, index) =>
        joinAsGuest(browser, invitation, `Guest ${index + 1}`),
      ),
    );
  ...
  test('FAST and CONFIRMATION payments preserve one command result after retry, then finish with final statistics', async ({
    browser,
  }) => {
  ...
  test('local second wallet, guest upgrade and keyboard/focus semantics remain usable', async ({
    browser,
  }) => {
  ...
  test('amount unit toggle drives the recorded transaction amount', async ({ browser }) => {
    ...
    await expect(
      owner.page.getByText(/enter a positive whole number|введіть додатне ціле число/i),
    ).toBeVisible();
...
test.describe('visual and accessibility matrix', () => {
  ...
  for (const visualCase of responsiveCases) {
    test(`major routes at ${visualCase.width}px ${visualCase.language} ${visualCase.colorScheme}`, async ({
      browser,
    }, testInfo) => {
      const context = await createVisualContext(browser, visualCase);
      const page = await context.newPage();
      await mockVisualApi(page);
      await exerciseMajorViews(
        page,
        visualCase.language,
        `${visualCase.width}-${visualCase.language}-${visualCase.colorScheme}`,
        testInfo,
      );
      await context.close();
    });
    ...
    for (const colorScheme of ['light', 'dark'] as const) {
      for (const reducedMotion of ['reduce', 'no-preference'] as const) {
        for (const visualStyle of ['classic-bank', 'liquid-glass'] as const) {
          test(`${visualStyle} ${language} ${colorScheme} ${reducedMotion}`, async ({
            browser,
          }, testInfo) => {
            ...
            await attachPage(
              page,
              testInfo,
              `${visualStyle}-${language}-${colorScheme}-${reducedMotion}-activity`,
            );
            ...
            await attachPage(
              page,
              testInfo,
              `${visualStyle}-${language}-${colorScheme}-${reducedMotion}-statistics`,
            );
            await page.keyboard.press('Escape');
            ...
            await attachPage(
              page,
              testInfo,
              `${visualStyle}-${language}-${colorScheme}-${reducedMotion}-settings`,
            );
            await context.close();
          });
        }
  ...
  for (const visualCase of liquidGlassQaCases) {
    test(`liquid-glass functional surfaces at ${visualCase.width}px ${visualCase.colorScheme}`, async ({
      browser,
    }) => {
      const context = await createVisualContext(browser, {
        ...visualCase,
  ...
  for (const width of [390, 1280] as const) {
    test(`liquid-glass saved-game menu exposes duplicate and delete at ${width}px`, async ({
      browser,
    }) => {
      const context = await createVisualContext(browser, {
        width,
      ...
      await page
        .getByRole('button', {
          name: translate('en', 'gameActions', { name: visualGame.name }),
          exact: true,
        })
        .click();
      await expect(
        page.getByRole('menuitem', { name: translate('en', 'duplicate'), exact: true }),
      ).toBeVisible();
      await expect(
        page.getByRole('menuitem', { name: translate('en', 'removeGame'), exact: true }),
      ).toBeVisible();
      await context.close();
    });
  }
  ...
  for (const visualCase of [
    ...
    test(`liquid-glass incoming payments at ${visualCase.width}px ${visualCase.colorScheme}`, async ({
      browser,
    }) => {
      const context = await createVisualContext(browser, {
        ...visualCase,
      ...
      await expect(
        page.getByRole('heading', { name: translate('en', 'paymentInbox'), exact: true }),
      ).toBeVisible();
      await assertViewportHealth(page);
  ...
  for (const width of [320, 390, 768, 1280, 1440] as const) {
    test(`liquid-glass settings remain contained after switching to Ukrainian at ${width}px`, async ({
      browser,
    }, testInfo) => {
      const context = await createVisualContext(browser, {
        width,
...
async function createVisualContext(
  browser: Browser,
  ...
  await context.addInitScript(
    ({ language, colorMode, visualStyle }) => {
      try {
        ...
        /* The initial opaque about:blank document does not expose storage. */
      }
    },
    {
      language: visualCase.language,
      colorMode: visualCase.colorScheme,
      visualStyle: visualCase.visualStyle ?? 'classic-bank',
    },
  );
  return context;
}

async function exerciseMajorViews(
  page: Page,
  language: Language,
  ...
  await expect(
    page.getByRole('heading', { name: translate(language, 'playerProfile'), exact: true }).first(),
  ).toBeVisible();
  await assertViewportHealth(page);
  ...
  await expect(
    page.getByRole('heading', { name: translate(language, 'joinGame'), exact: true }),
  ).toBeVisible();
  await assertViewportHealth(page);
  ...
  const activityTrigger = page.getByRole('button', {
    name: translate(language, 'activity'),
    exact: true,
  });
  await openActivity(page, language);
  if (language === 'uk') {
    await expect(
      page.getByText(
        translate(language, 'bankruptcyTransactionDescription', {
          source: visualPlayers[2].name,
          destination: visualPlayers[1].name,
        }),
        { exact: true },
      ),
    ).toBeVisible();
  ...
  await expect(
    page.getByRole('tab', { name: translate(language, 'activityMine'), exact: true }),
  ).toBeFocused();
  await page.locator('.activity-scroll').evaluate((element) => {
    element.scrollTop = element.scrollHeight;
  });
  await expect(
    page.getByRole('heading', { name: translate(language, 'activity'), exact: true }),
  ).toBeVisible();
  await page.keyboard.press('Escape');
  ...
  if (language === 'uk') {
    ...
    await expect(
      page.getByText(translate(language, 'durationHoursMinutes', { hours: 2, minutes: 40 }), {
        exact: true,
      }),
    ).toBeVisible();
  }
...
async function openActivity(page: Page, language: Language): Promise<void> {
  ...
  await expect(
    page
      .getByRole('dialog')
      .getByRole('heading', { name: translate(language, 'activity'), exact: true }),
  ).toBeVisible();
...
async function openStatistics(page: Page, language: Language): Promise<void> {
  ...
  await expect(
    page
      .getByRole('dialog')
      .getByRole('heading', { name: translate(language, 'statistics'), exact: true }),
  ).toBeVisible();
  await waitForOverlayMotion(page);
}
...
async function openSettings(page: Page, language: Language, assertAnchor = true): Promise<void> {
  ...
  await expect(
    page
      .getByRole('dialog')
      .getByRole('heading', { name: translate(language, 'settings'), exact: true }),
  ).toBeVisible();
  await waitForOverlayMotion(page);
  if (assertAnchor && (page.viewportSize()?.width ?? 0) >= 768)
    await assertSettingsAnchor(page, language);
}
...
async function exerciseLiquidGlassSurfaces(
  page: Page,
  language: Language,
  colorScheme: 'light' | 'dark',
  navigate = true,
...
async function assertFunctionalDialog(page: Page): Promise<void> {
  ...
  const styles = await dialog.evaluate((element) => {
    ...
    return {
      ...
      backdropFilter:
        backdropStyle.backdropFilter || backdropStyle.getPropertyValue('-webkit-backdrop-filter'),
    };
  });
...
async function attachPage(page: Page, testInfo: TestInfo, name: string): Promise<void> {
  ...
  const body = await page.screenshot({
    fullPage: true,
    ...(outputDirectory === undefined ? {} : { path: `${outputDirectory}/${name}.png` }),
  });
...
async function mockVisualApi(page: Page): Promise<void> {
  await page.route('**/api/**', async (route) => {
    ...
    else if (url.pathname === `/api/games/${visualGame.id}/activity`)
      data = url.searchParams.get('scope') === 'PENDING' ? visualPendingActivity : visualActivity;
...
async function mockActivePaymentInbox(page: Page): Promise<void> {
  await page.route(`**/api/games/${visualGame.id}/payment-requests`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ data: visualPendingActivity.paymentRequests }),
    });
  });
  await page.route(`**/api/games/${visualGame.id}`, async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        data: {
          ...visualGameDetails,
          game: { ...visualGameDetails.game, status: 'ACTIVE', finishedAt: null },
        },
      }),
    });
  });
...
const visualPlayers: Player[] = [
  {
    ...
    name: 'Oleksandra Very-Long-Double-Barrelled Rail Baron',
    color: '#d83f55',
    balance: 9_007_199_254_740_000,
    status: 'ACTIVE',
    userId: 'visual-user',
    createdAt: visualGame.createdAt,
  },
  {
    id: 'player-two',
    gameId: visualGame.id,
    name: 'Богдан Надзвичайно-Довге-Ім’я Власника Комунальних Служб',
    color: '#2878d0',
    balance: 888_888_888_888,
    status: 'ACTIVE',
    createdAt: visualGame.createdAt,
  },
  {
...
const visualProfile: UserProfile = {
  ...
  winRate: 0.55,
  createdAt: visualGame.createdAt,
  updatedAt: visualGame.updatedAt,
};
const visualGames: GameSummary[] = [{ game: visualGame, playerCount: visualPlayers.length }];
const visualGameDetails: GameDetails = {
  game: visualGame,
  players: visualPlayers,
  favoriteAmounts: [100, 500],
  recentAmounts: [2_000],
  controlledPlayerIds: ['player-one'],
  controlledWallets: [{ playerId: 'player-one', kind: 'PRIMARY' }],
  canManage: true,
};
...
const visualTransactions: Transaction[] = Array.from({ length: 48 }, (_, index) => {
  ...
  return {
    ...
    participants: [
      { playerId: index === 0 ? 'player-three' : 'player-one', balanceDelta: -amount },
      { playerId: 'player-two', balanceDelta: amount },
    ],
  };
});

const visualActivity: ActivityPage = {
  transactions: visualTransactions,
  paymentRequests: [],
  nextCursor: null,
};
const visualPendingActivity: ActivityPage = {
  transactions: [],
  paymentRequests: [
    {
      id: 'pending-1',
      gameId: visualGame.id,
      payerPlayerId: 'player-one',
      recipientPlayerId: 'player-two',
      creatorPlayerId: 'player-two',
      approverPlayerId: 'player-one',
      amount: 7_777_777_777,
      comment: 'Boardwalk settlement',
      state: 'PENDING',
      expiresAt: '2026-09-20T11:00:00.000Z',
      createdAt: '2026-09-20T10:38:00.000Z',
      resolvedAt: null,
      transactionId: null,
    },
  ],
...
const visualStatistics: { game: Game; winners: Player[] } & LedgerStatistics = {
  ...
  biggestPayerRecipient: {
    payerId: 'player-one',
    recipientId: 'player-two',
    amount: 555_555_555,
    transactionCount: 42,
  },
  cashLeaderboard: visualPlayers.map((player, index) => ({
    player,
    sent: 900_000_000 - index,
    received: 800_000_000 - index,
    passGoCount: 12 - index,
    transactionCount: 4_000 - index,
  })),
};
...
async function register(
  ...
  await page
    .getByLabel(/nickname|псевдонім/i)
    .fill(`${nickname} ${Date.now()} ${Math.random().toString(36).slice(2, 7)}`);
...
async function createLobby(
  ...
  for (let index = 0; index < localPlayers; index += 1) {
    ...
    await page
      .getByLabel(new RegExp(`player ${index + 2} name|ім.?я гравця ${index + 2}`, 'i'))
      .fill(`Local ${index + 1}`);
  }
...
async function createInvitation(page: Page): Promise<string> {
  ...
  await page
    .getByRole('button', { name: /copy invite link|копіювати посилання-запрошення/i })
    .click();
  const invitationUrl = await page.evaluate(() => navigator.clipboard.readText());
  await page
    .getByRole('button', { name: /close|закрити/i })
    .first()
    .click();
  if (!invitationUrl.includes('#invite='))
    throw new Error('The copied invitation is not a secure fragment URL.');
...
async function startLobby(page: Page): Promise<void> {
  await page.getByRole('button', { name: /start game|почати гру/i }).click();
}
async function openPayBank(page: Page): Promise<void> {
  await page.getByRole('button', { name: /open banking actions|відкрити банківські дії/i }).click();
  await page.getByRole('button', { name: /^pay bank$|^заплатити банку$/i }).click();
}
async function pressKeypadDigit(page: Page, digit: string): Promise<void> {
  await page
    .getByRole('group', { name: /numeric keypad|цифрова клавіатура/i })
    .getByRole('button', { name: digit, exact: true })
    .click();
}
async function readGameDetails(page: Page, gameId: string): Promise<GameDetailsResponse> {
  return (await page.evaluate(
    async (id) => (await fetch(`/api/games/${id}`)).json(),
    gameId,
  )) as GameDetailsResponse;
}
async function readPlayerTransactions(
  page: Page,
  gameId: string,
  playerId: string,
): Promise<{ data: Array<{ type: string; amount: number }> }> {
  return (await page.evaluate(
    async ({ id, player }) =>
      (await fetch(`/api/games/${id}/players/${player}/transactions`)).json(),
    { id: gameId, player: playerId },
  )) as { data: Array<{ type: string; amount: number }> };
}
function balanceOf(details: GameDetailsResponse, playerId: string): number {
  const player = details.data.players.find((candidate) => candidate.id === playerId);
  if (player === undefined) throw new Error(`Player ${playerId} is missing from the game details.`);
  return player.balance;
}
type GameDetailsResponse = {
  data: { controlledPlayerIds: string[]; players: Array<{ id: string; balance: number }> };
};
async function finishWithNoWinner(page: Page): Promise<void> {
  await page.getByRole('button', { name: /finish game|завершити гру/i }).click();
  await page.getByRole('button', { name: /no winner|без переможця/i }).click();
}
async function upgradeGuest(page: Page): Promise<void> {
  await page.getByRole('button', { name: /Guest 1/ }).click();
  await page
    .getByLabel(/email|електронна пошта/i)
    .last()
    .fill(`upgrade-${crypto.randomUUID()}@example.test`);
  await page
    .getByLabel(/^password$|^пароль$/i)
    .last()
    .fill('SafeE2EPassword123!');
  await page
    .getByRole('button', { name: /secure this guest account|захистити гостьовий обліковий запис/i })
    .click();
...
async function assertCommandIdempotency(page: Page, gameId: string): Promise<void> {
  ...
  const request = {
    type: 'PLAYER_TO_BANK',
    playerId: details.data.controlledPlayerIds[0],
    amount: 1,
  };
  const responses = await page.evaluate(
    async ({ id, request, commandId: idempotencyKey }) =>
      Promise.all(
        [0, 1].map(async () => {
          const response = await fetch(`/api/games/${id}/transactions`, {
            method: 'POST',
            headers: { 'content-type': 'application/json', 'x-command-id': idempotencyKey },
...
async function assertConfirmationPayment(owner: Page, guest: Page, gameId: string): Promise<void> {
  const [ownerDetails, guestDetails] = (await Promise.all(
    [owner, guest].map((page) =>
      page.evaluate(async (id) => (await fetch(`/api/games/${id}`)).json(), gameId),
    ),
  )) as Array<{ data: { controlledPlayerIds: string[] } }>;
  const request = (await owner.evaluate(
    async ({ id, sourcePlayerId, destinationPlayerId }) => {
      const response = await fetch(`/api/games/${id}/transactions`, {
        method: 'POST',
    ...
    {
      id: gameId,
      sourcePlayerId: ownerDetails.data.controlledPlayerIds[0],
      destinationPlayerId: guestDetails.data.controlledPlayerIds[0],
    },
  )) as { data: { paymentRequests: Array<{ id: string }> } };
  const paymentRequestId = request.data.paymentRequests[0]?.id;
  if (paymentRequestId === undefined)
    throw new Error('CONFIRMATION payment did not create a payment request.');
  const accepted = (await guest.evaluate(
    async ({ id, paymentRequestId: requestId }) => {
      const response = await fetch(`/api/games/${id}/payment-requests/${requestId}/accept`, {
        method: 'POST',
        headers: { 'x-command-id': crypto.randomUUID() },
      });
      return response.json();
    },
    { id: gameId, paymentRequestId },
  )) as { data: { transaction?: { id: string } } };
