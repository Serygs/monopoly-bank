...
export function AppearanceSettings({ preferences, onChange }: {
  ...
  const { t } = useLanguage();
  return <>
    <fieldset className="settings-options"><legend>{t('visualStyle')}</legend><div className="style-preview-grid">
      {visualStyles.map((style) => <button key={style.id} type="button" className={`style-preview-card style-preview-card--${style.id}${preferences.visualStyle === style.id ? ' active' : ''}`} aria-pressed={preferences.visualStyle === style.id} onClick={() => onChange({ visualStyle: style.id })}><span className="style-preview-card-art" aria-hidden="true" /><span>{t(style.labelKey)}</span><span className="style-preview-card-check" aria-hidden="true">✓</span></button>)}
    </div></fieldset>
