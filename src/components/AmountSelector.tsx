...
export function AmountSelector({
  ...
  const [state, setState] = useState<AmountInputState>(() =>
    syncCanonicalAmount({ digits: '', unit: readAmountUnit() }, value),
  );
    ...
    <div className="amount-selector">
      <div className="dialog-field amount-field">
        <label htmlFor={inputId}>
          {t('amount')} <span className="field-note">{t(unitLabels[synced.unit].note)}</span>
        </label>
        <div className="amount-field-row">
          <span className="amount-input-wrap">
            <input
              ...
              inputMode="numeric"
              pattern="[0-9 ]*"
              value={synced.digits === '' ? '' : formatThousands(Number(synced.digits))}
              onChange={(event) =>
                changeDigits(sanitizeMoneyInput(event.target.value, synced.unit))
              }
            />
            <span className="amount-unit-suffix" aria-hidden="true">
              {t(unitLabels[synced.unit].short)}
            </span>
          </span>
          <div className="amount-unit-toggle" role="group" aria-label={t('amountUnitGroup')}>
            {units.map((unit) => (
              <button
                key={unit}
                type="button"
                aria-pressed={synced.unit === unit}
                aria-label={t(unitLabels[unit].full)}
                onClick={() => changeUnit(unit)}
              >
                {t(unitLabels[unit].short)}
              </button>
            ))}
          </div>
        </div>
