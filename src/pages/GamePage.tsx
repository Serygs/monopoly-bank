...
const primaryWalletActions: ActionType[] = [
  'PLAYER_TO_PLAYER',
  'PLAYER_TO_BANK',
  'BANK_TO_PLAYER',
  'PASS_GO',
...
export function GamePage({ gameId, onBack, preferences, offline, onPaymentFlowChange }: Props) {
  ...
  const [liveStatus, setLiveStatus] = useState<'connecting' | 'reconnecting' | 'live' | 'offline'>(
    'connecting',
  );
            ...
            <div>
              {controlledWallets.map((wallet) => {
                const player = details.players.find(
                  (candidate) => candidate.id === wallet.playerId,
                );
          ...
          {activeWallet !== null && (
            ...
            >
              ...
              <WalletCard
                ...
                prominent
                onOpen={() => setSelectedPlayer(activeWallet)}
              />
            </section>
          )}
...
function PaymentInbox({
  ...
  players: Player[];
  currency: Currency;
  onAction: (request: PaymentRequest, action: 'accept' | 'decline') => Promise<void>;
}) {
  const { t, locale } = useLanguage();
  const [busyId, setBusyId] = useState<string | null>(null);
  const [error, setError] = useState<unknown | null>(null);
  const act = async (request: PaymentRequest, action: 'accept' | 'decline') => {
        ...
        <ol className="history-list">
          {history.map((transaction) => (
            <li key={transaction.id}>
              <strong>{transactionDescription(transaction, players, language)}</strong>
              <span className={player === null ? '' : 'player-history-amount'}>
                {player === null
                  ? transactionAmount(transaction, currency, language)
                  : playerTransactionAmount(transaction, player.id, currency)}
              </span>
              <small>
              ...
              {transaction.comment !== null && <em>{transaction.comment}</em>}
            </li>
          ))}
        </ol>
      )}
...
function controllerLabel(kind: PlayerControllerKind, t: Translate): string {
    ...
    <fieldset className="player-picker">
      ...
      <div>
        {players.map((candidate) => (
          ...
          >
            <span className="player-color" style={{ backgroundColor: candidate.color }} />
            <span>{playerNameWithGameId(candidate, gamePlayers)}</span>
            <small>{formatMoney(candidate.balance, currency)}</small>
          </button>
        ))}
      </div>
    </fieldset>
...
function Confirmation({
  ...
  passGoReward: number;
  currency: Currency;
  preview: { player: Player; after: number; delta: number }[];
  comment: string;
}) {
  const { t } = useLanguage();
  const total =
