...
export function GamePage({ gameId, onBack, preferences, offline, onPaymentFlowChange }: Props) {
  ...
  return <main className="page game-page">
    <PageHeader className="game-heading" eyebrow={t(gameStatusTranslationKey(details.game.status))} title={details.game.name} description={t('configuredPassGoReward', { amount: formatMoney(details.game.passGoReward, details.game.currency) })} backAction={<button className="button button-quiet" type="button" onClick={onBack}>{t('savedGames')}</button>} actions={<>{details.canManage && details.game.status === 'LOBBY' && <button className="button button-secondary" type="button" disabled={offline} onClick={() => setInviteOpen(true)}>{t('invitePlayers')}</button>}{details.canManage && details.game.status === 'ACTIVE' && <button className="button button-danger" type="button" disabled={offline} onClick={() => setFinishing(true)}>{t('finishGame')}</button>}<button className="button button-secondary" type="button" disabled={offline} onClick={() => setActivityOpen(true)}>{t('activity')}</button><button className="button button-secondary" type="button" disabled={offline} onClick={() => void monopolyBankApi.getGameSummary(gameId).then((result) => { setSummary(result); setStatisticsOpen(true); })}>{t('statistics')}</button></>}>
