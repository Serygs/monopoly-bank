...
export function VerifyEmailPage({ token, onVerified, onBack }: { token: string; onVerified: (profile: UserProfile) => void; onBack: () => void }) {
  ...
  useEffect(() => { let active = true; const verify = async () => { try { const profile = await monopolyBankApi.verifyEmail(token); if (active) { onVerified(profile); setVerified(true); } } catch (caught) { if (active) setError(caught); } }; void verify(); return () => { active = false; }; }, [onVerified, token]);
...
export function PasswordResetPage({ token, onBack }: { token: string; onBack: () => void }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); if (password.length < 6) { setError(new Error(t('accountPasswordMinLength'))); return; } setBusy(true); setError(null); try { await monopolyBankApi.confirmPasswordReset(token, password); setChanged(true); } catch (caught) { setError(caught); } finally { setBusy(false); } };
