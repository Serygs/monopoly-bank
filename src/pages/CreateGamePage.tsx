...
export function CreateGamePage({ onCancel, onCreated }: Props) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true); try { const details = await monopolyBankApi.createGame({ name: name.trim(), startingBalance: Number(startingBalance), passGoReward: Number(passGoReward), currency, players: players.map((player) => ({ name: player.name.trim(), color: player.color })) }); onCreated(details.game.id); } catch (caught) { setSubmitError(caught); } finally { setSubmitting(false); }
  };
