...
export function CreateGamePage({ profile, onCancel, onCreated }: Props) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true); try { const details = await monopolyBankApi.createGame({ name: name.trim(), startingBalance: Number(startingBalance), passGoReward: Number(passGoReward), currency, paymentMode, ...(boardId === null ? {} : { boardId }), ...(gameAccessPassword === '' ? {} : { gameAccessPassword }), players: players.map((player) => ({ name: player.name.trim(), color: player.color })) }); onCreated(details.game.id); } catch (caught) { setSubmitError(caught); } finally { setSubmitting(false); }
  };
  return <main className="page create-game-page"><PageHeader eyebrow={t('newBank')} title={t('createLobby')} description={t('lobbyLede')} backAction={<button className="button button-quiet" type="button" onClick={onCancel}>{t('cancel')}</button>} />
    <form className="game-form create-game-form" onSubmit={(event) => void submit(event)} noValidate>
      <fieldset className="form-section create-section-basics"><legend>{t('gameBasics')}</legend><div className="form-section-fields"><label>{t('gameName')}<input value={name} onChange={(event) => setName(event.target.value)} aria-invalid={errors.name !== undefined} />{fieldError(errors.name)}</label><label>{t('currency')}<select value={currency} onChange={(event) => setCurrency(event.target.value as SelectableCurrency)}>{selectableCurrencies.map((option) => <option key={option} value={option}>{option}</option>)}</select></label></div></fieldset>
      ...
      <fieldset className="form-section create-section-board"><legend>{t('boardSection')}</legend><div className="form-section-fields"><BoardPicker boards={boards} loading={boardsLoading} error={boardsError} value={boardId} onChange={setBoardId} onCreateCustom={() => setCustomBoardOpen(true)} />{boardNotice !== null && <p className="notice notice-success board-created" role="status">{boardNotice}</p>}</div></fieldset>
