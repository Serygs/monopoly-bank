import type { DiceRollResponse, GameDetails } from '../shared/contracts/api.js';
import { isLiveMutationCommand, type JailChange, type LiveGameState, type LiveMutationCommand, type LiveServerEvent } from '../shared/contracts/live.js';
import { controlledPlayerIdForBankingCommand, rentBankingCommand } from '../shared/domain/player-control.js';
import type { BuildingBank, GameProperty, Player, Transaction } from '../shared/types/monopoly.js';
...
export class GameSession {
  ...
  private startCommand(gameId: string, userId: string, command: LiveMutationCommand, payloadHash: string): Promise<ResponsePayload> {
    const pending = (async () => {
      ...
      // The deeds as they stand before a command that may move them, so the event can carry only the rows it changed.
      const deedsBefore = movesDeeds(command) ? await this.deeds().listProperties(gameId) : null;
      ...
      await this.publish(command, response.data, stateVersion, deedsBefore === null ? undefined : boardCommit(deedsBefore, response.data));
  ...
  /**
   * One command, one authoritative fan-out. A commit that moved money is a
   * `GAME_COMMITTED`; when it also moved deeds, the changed rows ride along
   * (`board`), and a bail carries the jail flag it cleared. Offers and rolls are
   * not ledger commits and get their own events.
   */
  private async publish(command: LiveMutationCommand, data: unknown, stateVersion: number, board?: BoardCommit): Promise<void> {
    ...
    if (command.type === 'DICE_ROLL') { const roll = data as DiceRollResponse; await this.broadcast({ type: 'DICE_ROLLED', stateVersion, player: roll.player, thirdDouble: roll.thirdDouble }); return; }
    ...
    if (result.transaction !== undefined && result.players !== undefined) {
      await this.broadcast({
        ...
        ...(command.type === 'DECLARE_BANKRUPTCY' ? { bankruptPlayerId: command.request.playerId } : {}),
        ...(board ?? {}),
        ...(command.type === 'JAIL_BAIL' ? { jailChanges: jailChangesFor(result.players, command.request.playerId) } : {}),
      });
    }
    ...
    if (command.type === 'PROPOSE_TRADE' || command.type === 'RESOLVE_TRADE') await this.broadcast({ type: 'TRADES_UPDATED', stateVersion });
  }
  ...
  private deeds(): Pick<D1PropertyRepository, 'listProperties'> { return new D1PropertyRepository(this.env.MONOPOLY_BANK_DB); }
...
/** The board part of a `GAME_COMMITTED`: only the deed rows the commit changed, plus the bank as it now stands. */
interface BoardCommit { properties: GameProperty[]; buildingBank?: BuildingBank; }

/** The commands whose result may carry a full deed slice worth diffing; a plain money move never reads the deeds. */
function movesDeeds(command: LiveMutationCommand): boolean { return command.type === 'PROPERTY_OPERATION' || command.type === 'RESOLVE_TRADE' || command.type === 'DECLARE_BANKRUPTCY'; }

/**
 * Reduces the full slice a service answers with to the rows that differ from
 * the deeds read before the command ran. A result without a slice (a declined
 * trade, a rent that became a payment request) yields no board part at all.
 */
function boardCommit(before: readonly GameProperty[], data: unknown): BoardCommit | undefined {
  if (data === null || typeof data !== 'object' || !Array.isArray((data as { properties?: unknown }).properties)) return undefined;
  const result = data as { properties: GameProperty[]; buildingBank?: BuildingBank };
  const previous = new Map(before.map((property) => [property.boardSpaceId, property]));
  const properties = result.properties.filter((property) => {
    const known = previous.get(property.boardSpaceId);
    return known === undefined || known.ownerPlayerId !== property.ownerPlayerId || known.houses !== property.houses || known.mortgaged !== property.mortgaged;
  });
  return { properties, ...(result.buildingBank === undefined ? {} : { buildingBank: result.buildingBank }) };
}

function jailChangesFor(players: readonly Player[], playerId: string): JailChange[] {
  const player = players.find((candidate) => candidate.id === playerId);
  return [{ playerId, isInJail: player?.isInJail ?? false }];
}
