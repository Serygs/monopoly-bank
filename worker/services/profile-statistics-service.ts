...
export class ProfileStatisticsService {
  ...
  async recordCompletedGame(gameId: string, participants: readonly { userId: string; won: boolean }[]): Promise<void> {
    for (const participant of participants) if (await this.completions.record(gameId, participant.userId, participant.won)) await this.completions.incrementProfile(participant.userId, participant.won);
  }
}
