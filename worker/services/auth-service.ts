...
export class AuthService {
  ...
  async register(nickname: string, avatar: string, password: string): Promise<{ profile: PublicProfile; cookie: string }> { if (await this.users.findByNickname(nickname) !== null) throw new ConflictError('NICKNAME_ALREADY_EXISTS', 'Nickname is already in use.'); const credentials = await hashPassword(password); const user = await this.users.create({ id: this.createId(), nickname, avatar, passwordHash: credentials.hash, passwordSalt: credentials.salt }); return { profile: profile(user), cookie: await this.createSession(user.id) }; }
  async login(nickname: string, password: string): Promise<{ profile: PublicProfile; cookie: string }> { const user = await this.users.findByNickname(nickname); if (user === null || !(await verifyPassword(password, { hash: user.passwordHash, salt: user.passwordSalt }))) throw new InvalidCredentialsError(); return { profile: profile(user), cookie: await this.createSession(user.id) }; }
  async current(request: Request): Promise<PublicProfile> { const token = cookieValue(request.headers.get('cookie'), 'monopoly_bank_session'); if (token === null) throw new AuthenticationRequiredError(); const userId = await this.sessions.findUserId(await tokenHash(token)); if (userId === null) throw new AuthenticationRequiredError(); const user = await this.users.findById(userId); if (user === null) throw new AuthenticationRequiredError(); return profile(user); }
