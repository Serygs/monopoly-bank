...
export class AuthService {
  ...
  async register(input: RegisterRequest): Promise<{ profile: PublicProfile; cookie: string }> {
    if ((await this.users.findByNickname(input.nickname)) !== null)
  ...
  async update(userId: string, nickname: string, avatar: string): Promise<PublicProfile> {
    const updated = await this.users.updateProfile(userId, nickname, avatar);
    if (updated === null) throw new ResourceNotFoundError('User');
    return profile(updated);
  }
  async verifyEmail(token: string): Promise<PublicProfile> {
  ...
  async requestPasswordReset(email: string): Promise<void> {
    ...
    if (user?.accountType === 'REGISTERED' && user.email !== null)
      await this.issueEmailTokenBestEffort(user, 'RESET_PASSWORD');
  }
  async resetPassword(token: string, password: string): Promise<void> {
    const userId = await this.tokens.consume(await tokenHash(token), 'RESET_PASSWORD');
    if (userId === null)
      throw new ValidationError('This password reset link is invalid or has expired.');
    const credentials = await hashPassword(password);
    const user = await this.users.changePassword(userId, credentials.hash, credentials.salt);
    if (user === null || user.accountType !== 'REGISTERED') throw new ResourceNotFoundError('User');
    await this.sessions.deleteAllForUser(user.id);
  }
  async resendVerification(userId: string): Promise<void> {
    const user = await this.users.findById(userId);
    if (user === null) throw new ResourceNotFoundError('User');
    if (user.accountType !== 'REGISTERED' || user.email === null || user.emailVerifiedAt !== null)
      return;
    await this.issueEmailToken(user, 'VERIFY_EMAIL', true);
  }
  async upgradeGuest(
    userId: string,
    email: string,
    ...
    const user = await this.users.upgradeGuest({
      id: userId,
      email,
      normalizedEmail,
      passwordHash: credentials.hash,
    ...
    if (user === null)
      throw new ConflictError(
        'GUEST_UPGRADE_UNAVAILABLE',
        'This guest account can no longer be upgraded.',
      );
  ...
  async addEmailToLegacyAccount(userId: string, email: string): Promise<PublicProfile> {
    ...
    if (user === null)
      throw new ConflictError(
        'EMAIL_MIGRATION_UNAVAILABLE',
        'An email cannot be added to this account.',
      );
    await this.issueEmailTokenBestEffort(user, 'VERIFY_EMAIL');
    return profile(user);
  }
  async logout(request: Request): Promise<string> {
    const token = cookieValue(request.headers.get('cookie'), 'monopoly_bank_session');
    if (token !== null) await this.sessions.delete(await tokenHash(token));
    return expiredCookie();
  }
  async revokeAll(userId: string): Promise<string> {
    await this.sessions.deleteAllForUser(userId);
    return expiredCookie();
  }
  async cleanupExpired(): Promise<void> {
    await Promise.all([this.sessions.deleteExpired(), this.tokens.deleteExpired()]);
  }
  async exportAccount(userId: string): Promise<AccountExport> {
    if (this.privacy === undefined) throw new ResourceNotFoundError('Account export');
    return this.privacy.exportAccount(userId);
  }
  async deleteAccount(userId: string): Promise<string> {
    ...
    if (await this.privacy.hasActiveMembership(userId))
      throw new ConflictError(
        'ACCOUNT_DELETE_ACTIVE_GAME',
        'Finish or leave active games before deleting this account.',
      );
    const credentials = await hashPassword(randomToken());
    if (
      !(await this.privacy.anonymizeAccount(
        userId,
        `deleted-${userId}`,
        credentials.hash,
        credentials.salt,
  ...
  private async issueEmailToken(
    ...
    const subject = verify
      ? 'Verify your Monopoly Bank email'
      : 'Reset your Monopoly Bank password';
    ...
    try {
      await this.email.send({
        ...
        subject,
        text,
        html: `<p><a href="${escapeHtml(url.toString())}">${escapeHtml(verify ? 'Verify your email' : 'Reset your password')}</a></p>`,
        idempotencyKey: `${purpose}:${hash}`,
      });
    } catch (cause) {
  ...
  private async issueEmailTokenBestEffort(
    user: UserRecord,
    purpose: AuthTokenPurpose,
  ): Promise<void> {
    try {
      await this.issueEmailToken(user, purpose);
    } catch {
      /* A provider outage must not invalidate an existing session or guest flow. */
    }
  }
...
function profile(user: UserRecord): PublicProfile {
  return {
    id: user.id,
    ...
    winRate: user.gamesPlayed === 0 ? 0 : user.gamesWon / user.gamesPlayed,
    createdAt: user.createdAt,
    updatedAt: user.updatedAt,
  };
}
function cookieValue(value: string | null, name: string): string | null {
  return (
    value
      ?.split(';')
      .map((item) => item.trim())
