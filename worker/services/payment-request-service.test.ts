...
const game: Game = { id: 'game', name: 'Table', startingBalance: 500, passGoReward: 200, currency: 'K', paymentMode: 'CONFIRMATION', status: 'ACTIVE', createdAt: '', updatedAt: '' };
...
describe('confirmation payment requests', () => {
  it('reserves a direct payment without ledger write and settles exactly once on repeat accept', async () => {
    ...
    const service = new DefaultBankingService({
      ...
      transactions: { getById: async (_gameId: string, id: string) => transactions.get(id) ?? null } as never,
      operations: operations as never,
      paymentRequests: {
        create: async (request: Omit<PaymentRequest, 'createdAt' | 'resolvedAt' | 'transactionId'>) => { requests.set(request.id, { ...request, createdAt: '', resolvedAt: null, transactionId: null }); },
        getById: async (_gameId: string, id: string) => requests.get(id) ?? null,
        pendingReservedAmount: async () => [...requests.values()].filter((request) => request.state === 'PENDING').reduce((sum, request) => sum + request.amount, 0),
        expirePending: async () => undefined,
        listForPayers: async () => [],
        settle: async ({ requestId, transactionId, transaction, balanceChanges }) => {
          const request = requests.get(requestId); if (request === undefined || request.state !== 'PENDING') throw new Error('already settled');
    ...
    const created = await service.createTransaction(game.id, { type: 'PLAYER_TO_PLAYER', sourcePlayerId: 'payer', destinationPlayerId: 'recipient', amount: 150 });
  ...
  it('does not settle when the payer no longer has funds', async () => {
    const request: PaymentRequest = { id: 'request', gameId: game.id, payerPlayerId: 'payer', recipientPlayerId: 'recipient', creatorPlayerId: 'payer', approverPlayerId: 'recipient', amount: 100, comment: null, state: 'PENDING', expiresAt: '2999-01-01T00:00:00.000Z', createdAt: '', resolvedAt: null, transactionId: null };
    players[0]!.balance = 50;
    const settle = vi.fn();
    const service = new DefaultBankingService({ games: { getById: async () => game } as never, players: { listByGameId: async () => players } as never, transactions: {} as never, operations: {} as never, paymentRequests: { getById: async () => request, expirePending: async () => undefined, settle, create: async () => undefined, listForPayers: async () => [], pendingReservedAmount: async () => 0, resolve: async () => true }, createId: () => 'transaction' });
    await expect(service.acceptPaymentRequest(game.id, request.id)).rejects.toMatchObject({ code: 'INSUFFICIENT_FUNDS' });
