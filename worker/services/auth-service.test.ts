...
describe('AuthService email accounts and guests', () => {
  it('registers an account from just a nickname and password, without an email', async () => {
    ...
    ).resolves.toMatchObject({ profile: { id: result.profile.id } });
  });
  ...
  it('upgrades a guest in place, normalizing and storing only a token hash, and sends verification', async () => {
    ...
    const upgraded = await fixture.auth.upgradeGuest(
      guest.profile.id,
      ' Ada@Example.TEST ',
      'secure-password',
    );
    ...
    const rawToken =
      new URL(fixture.email.messages[0].text.split(': ')[1]).hash.split('=')[1] ?? null;
  ...
  it('returns the same invalid-credentials result for unknown email and bad password', async () => {
    ...
    await expect(
      fixture.auth.login({ email: 'missing@example.test', password: 'secure-password' }),
    ).rejects.toBeInstanceOf(InvalidCredentialsError);
    await expect(
      fixture.auth.login({ email: 'ada@example.test', password: 'wrong-password' }),
    ).rejects.toBeInstanceOf(InvalidCredentialsError);
  });
  ...
  it('returns normally for known and unknown password-reset emails without sending to the unknown address', async () => {
    ...
    await expect(
      fixture.auth.requestPasswordReset('missing@example.test'),
  ...
  it('consumes a reset token and revokes all sessions after changing the password', async () => {
    const fixture = createFixture();
    const guest = await fixture.auth.createGuest('Ada', '🎩');
    const registered = await fixture.auth.upgradeGuest(
      guest.profile.id,
      'ada@example.test',
      'secure-password',
    );
    ...
    await fixture.auth.resetPassword(
    ...
    await expect(
      fixture.auth.login({ email: 'ada@example.test', password: 'secure-password' }),
    ).rejects.toBeInstanceOf(InvalidCredentialsError);
    await expect(
      fixture.auth.login({ email: 'ada@example.test', password: 'new-secure-password' }),
    ).resolves.toMatchObject({ profile: { id: registered.profile.id } });
  });
  ...
  it('keeps registration and guest upgrade available when email delivery is unavailable', async () => {
    ...
    await expect(
      fixture.auth.register({ nickname: 'Ada', avatar: 'рџЋ©', password: 'secure-password' }),
    ).resolves.toMatchObject({ profile: { accountType: 'REGISTERED' } });
    const guest = await fixture.auth.createGuest('Guest', 'рџЋІ');
    await expect(
      fixture.auth.upgradeGuest(guest.profile.id, 'guest@example.test', 'secure-password'),
    ).resolves.toMatchObject({ profile: { accountType: 'REGISTERED' } });
  });
  ...
  it('reports resend delivery failures and permits another retry', async () => {
    ...
    await expect(fixture.auth.resendVerification(registered.profile.id)).rejects.toBeInstanceOf(
      EmailDeliveryError,
    );
...
class MemoryUsers implements UserRepository {
  ...
  readonly byNickname = new Map<string, UserRecord>();
  ...
  async updateProfile(id: string, nickname: string, avatar: string): Promise<UserRecord | null> {
    ...
    this.byNickname.delete(user.nickname);
  ...
  async markEmailVerified(id: string): Promise<UserRecord | null> {
    const user = this.byId.get(id);
    if (user === undefined) return null;
    const updated = { ...user, emailVerifiedAt: '2026-01-01T00:00:00Z' };
    this.save(updated);
    return updated;
  }
  async changePassword(
    ...
    const user = this.byId.get(id);
    if (user === undefined) return null;
    const updated = { ...user, passwordHash, passwordSalt };
    this.save(updated);
    return updated;
  }
  async upgradeGuest(input: {
    ...
    if (
      ...
      this.byEmail.has(input.normalizedEmail)
    )
      return null;
    const updated = {
      ...user,
  ...
  private save(user: UserRecord): void {
    ...
    this.byNickname.set(user.nickname, user);
    if (user.normalizedEmail !== null) this.byEmail.set(user.normalizedEmail, user);
  }
}
...
class MemoryTokens implements AuthTokenRepository {
  readonly issued: Array<{ userId: string; purpose: AuthTokenPurpose; tokenHash: string }> = [];
  readonly active = new Map<string, string>();
  nextConsumedUserId: string | null = null;
  async issue(input: {
    id: string;
    userId: string;
    ...
    if (this.active.has(key) && input.replaceActive !== true) return false;
    this.active.set(key, input.tokenHash);
    this.issued.push(input);
    return true;
  }
  async consume(_tokenHash: string, purpose: AuthTokenPurpose): Promise<string | null> {
    const userId = this.nextConsumedUserId;
    if (userId !== null) this.active.delete(`${userId}:${purpose}`);
    this.nextConsumedUserId = null;
    return userId;
  }
  async discard(tokenHash: string): Promise<void> {
    for (const [key, activeHash] of this.active)
      if (activeHash === tokenHash) this.active.delete(key);
  }
  async deleteExpired(): Promise<void> {}
}
class MemoryEmail implements TransactionalEmailProvider {
