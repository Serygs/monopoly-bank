...
export function requireSameOriginForMutation(request: Request): void {
  ...
  if ((origin !== null && origin !== expectedOrigin) || fetchSite === 'cross-site') throw new ForbiddenError();
}
...
export function securityHeaders(request: Request, headers: Headers): void {
  ...
  headers.set('content-security-policy', "default-src 'self'; base-uri 'self'; frame-ancestors 'none'; form-action 'self'; object-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' wss:; manifest-src 'self'; worker-src 'self'");
