  ...
  return async (request: Request): Promise<Response> => {
    ...
    try {
      try {
        ...
        securityEvent('csrf_rejected', { requestId });
        throw error;
      }
...
async function route(
  ...
  const paymentRequestActionMatch =
    /^\/api\/games\/([^/]+)\/payment-requests\/([^/]+)\/(accept|decline|cancel)$/.exec(pathname);
  const playerTransactionMatch = /^\/api\/games\/([^/]+)\/players\/([^/]+)\/transactions$/.exec(
    pathname,
  );
  ...
  if (pathname === '/api/auth/verify-email' && request.method === 'POST')
    return success(
      await dependencies.auth.verifyEmail((await parseAuthTokenRequest(request)).token),
    );
  if (pathname === '/api/auth/password-reset' && request.method === 'POST') {
    await dependencies.auth.requestPasswordReset((await parsePasswordResetRequest(request)).email);
    return success({ accepted: true }, 202);
  }
  if (pathname === '/api/auth/password-reset/confirm' && request.method === 'POST') {
    const body = await parsePasswordResetConfirmationRequest(request);
    await dependencies.auth.resetPassword(body.token, body.password);
    return success(null);
  }
  if (pathname === '/api/auth/logout' && request.method === 'POST')
    return success(null, 200, await dependencies.auth.logout(request));
  ...
  if (pathname === '/api/games/join/guest' && request.method === 'POST') {
    ...
    const color = nextPlayerColor(details.players);
    if (dependencies.live !== undefined) {
      const response = await dependencies.live.mutate(access.gameId, profile.profile, {
        type: 'JOIN_LOBBY',
        commandId: readCommandId(request),
        playerId,
        nickname: profile.profile.nickname,
        color,
        startingBalance: details.game.startingBalance,
      });
    ...
    await dependencies.access.joinLobby({
    ...
    return success(
      { profile: profile.profile, game: await dependencies.games.getGame(access.gameId) },
      201,
      profile.cookie,
    );
  }
  ...
  if (pathname === '/api/auth/upgrade' && request.method === 'POST') {
    const body = await parseUpgradeGuestRequest(request);
    const result = await dependencies.auth.upgradeGuest(actor.id, body.email, body.password);
    return success(result.profile, 200, result.cookie);
  }
  if (pathname === '/api/auth/email' && request.method === 'POST')
    return success(
      await dependencies.auth.addEmailToLegacyAccount(
        actor.id,
        (await parseAddAccountEmailRequest(request)).email,
      ),
    );
  if (pathname === '/api/auth/verification-email' && request.method === 'POST') {
    await dependencies.auth.resendVerification(actor.id);
    return success({ accepted: true }, 202);
  }
  if (pathname === '/api/telemetry/pwa' && request.method === 'POST') {
    const body = (await request.json()) as { event?: unknown };
    if (
      body.event !== 'UPDATE_AVAILABLE' &&
      body.event !== 'UPDATE_APPLIED' &&
      body.event !== 'OFFLINE_SHELL'
    )
      throw new ApiValidationError('Unsupported PWA telemetry event.');
  ...
  if (pathname === '/api/profile') {
    ...
    if (request.method === 'PATCH') {
      const body = await parseUpdateProfileRequest(request);
      return success(await dependencies.auth.update(actor.id, body.nickname, body.avatar));
    }
  }
  if (pathname === '/api/games/join' && request.method === 'POST') {
    ...
    const color = nextPlayerColor(details.players);
    if (dependencies.live !== undefined)
      return dependencies.live.mutate(access.gameId, actor, {
        type: 'JOIN_LOBBY',
        commandId: readCommandId(request),
        playerId,
  ...
  if (pathname === '/api/games') {
    ...
    if (request.method === 'POST') {
      return success(
        await dependencies.games.createGameForOwner(
          actor.id,
          await parseCreateGameRequest(request),
        ),
        201,
      );
    }
  ...
  if (gameMatch !== null && request.method === 'GET') {
    const gameId = parseResourceId(gameMatch[1], 'gameId');
    const role = await dependencies.access.requireMember(gameId, actor.id);
    const controlledWallets = await dependencies.access.controlledWallets(gameId, actor.id);
    return success({
      ...(await dependencies.games.getGame(gameId)),
      controlledWallets,
      controlledPlayerIds: controlledWallets.map((wallet) => wallet.playerId),
      canManage: role === 'OWNER',
      ...(role === 'OWNER'
        ? { joinCode: await dependencies.access.ownerJoinCode(gameId, actor.id) }
        : {}),
    });
  }
  ...
  if (liveMatch !== null && request.method === 'GET') {
    ...
    try {
      requireSameOriginWebSocket(request);
    } catch (error) {
      securityEvent('websocket_origin_rejected', { requestId, gameId });
      throw error;
    }
    await dependencies.access.requireMember(gameId, actor.id);
    const details = await dependencies.games.getGame(gameId);
    if (details.game.status === 'FINISHED')
      return failure(409, 'GAME_FINISHED', 'This game is finished.', requestId);
    return dependencies.live === undefined
      ? failure(503, 'LIVE_UNAVAILABLE', 'Live games are not configured.', requestId)
      : dependencies.live.connect(gameId, actor, request);
  }

  if (gameMatch !== null && request.method === 'DELETE') {
    const gameId = parseResourceId(gameMatch[1], 'gameId');
    await dependencies.access.requireOwner(gameId, actor.id);
    return success(await dependencies.games.deleteGame(gameId));
  }
  if (invitationMatch !== null && request.method === 'POST') {
    const gameId = parseResourceId(invitationMatch[1], 'gameId');
    await dependencies.access.requireOwner(gameId, actor.id);
    return success(await dependencies.access.createInvitation(gameId, actor.id), 201);
  }
  if (invitationMatch !== null && request.method === 'DELETE') {
    const gameId = parseResourceId(invitationMatch[1], 'gameId');
    await dependencies.access.requireOwner(gameId, actor.id);
    await dependencies.access.revokeInvitations(gameId, actor.id);
    return success({ revoked: true });
  }
  if (summaryMatch !== null && request.method === 'GET') {
    const gameId = parseResourceId(summaryMatch[1], 'gameId');
    await dependencies.access.requireMember(gameId, actor.id);
    const details = await dependencies.games.getGame(gameId);
    if (dependencies.statistics === undefined)
      throw new ConflictError('STATISTICS_UNAVAILABLE', 'Statistics are not configured.');
    const snapshot =
      details.game.status === 'FINISHED'
        ? await dependencies.statistics.getFinalSnapshot(gameId)
        : null;
    const summary =
      snapshot?.summary ?? (await dependencies.statistics.calculate(details.game, details.players));
    const winners = (snapshot?.winnerPlayerIds ?? [])
      .map((id) => details.players.find((player) => player.id === id))
      .filter((player): player is (typeof details.players)[number] => player !== undefined);
    return success({ game: details.game, winners, ...summary });
  }
  if (pathname === '/api/account/export' && request.method === 'GET')
    return success(await dependencies.auth.exportAccount(actor.id));
  if (pathname === '/api/account' && request.method === 'DELETE')
    return success(null, 200, await dependencies.auth.deleteAccount(actor.id));
  if (duplicateMatch !== null && request.method === 'POST') {
    const gameId = parseResourceId(duplicateMatch[1], 'gameId');
    await dependencies.access.requireOwner(gameId, actor.id);
    return success(
      await dependencies.games.duplicateGameForOwner(
        actor.id,
        gameId,
        (await parseDuplicateGameRequest(request)).gameAccessPassword,
      ),
      201,
    );
  }
  if (startMatch !== null && request.method === 'POST') {
    const gameId = parseResourceId(startMatch[1], 'gameId');
    await dependencies.access.requireOwner(gameId, actor.id);
    if (dependencies.live !== undefined)
      return dependencies.live.mutate(gameId, actor, {
        type: 'START_GAME',
        commandId: readCommandId(request),
      });
    return success(await dependencies.games.startGame(gameId));
  }
  if (finishMatch !== null && request.method === 'POST') {
    const gameId = parseResourceId(finishMatch[1], 'gameId');
    await dependencies.access.requireOwner(gameId, actor.id);
    const body = await parseFinishGameRequest(request);
    const existing = await dependencies.games.getGame(gameId);
    if (body.winnerPlayerIds.some((id) => !existing.players.some((player) => player.id === id)))
      throw new ApiValidationError('winnerPlayerIds must belong to this game.');
    if (dependencies.live !== undefined)
      return dependencies.live.mutate(gameId, actor, {
        type: 'FINISH_GAME',
        commandId: readCommandId(request),
        winnerPlayerIds: body.winnerPlayerIds,
      });
    if (dependencies.statistics === undefined)
      throw new ConflictError('STATISTICS_UNAVAILABLE', 'Statistics are not configured.');
    await dependencies.statistics.saveFinalSnapshot(
      gameId,
      body.winnerPlayerIds,
      await dependencies.statistics.calculate(existing.game, existing.players),
    );
    const details = await dependencies.games.finishGame(gameId);
    const winnerIds = new Set(body.winnerPlayerIds);
    const participants = (await dependencies.access.linkedMembers(gameId))
      .filter((member) => member.playerId !== null)
      .map((member) => ({ userId: member.userId, won: winnerIds.has(member.playerId as string) }));
    await dependencies.profileStatistics.recordCompletedGame(gameId, participants);
    return success(details);
  }
  if (activityMatch !== null && request.method === 'GET') {
    const gameId = parseResourceId(activityMatch[1], 'gameId');
    await dependencies.access.requireMember(gameId, actor.id);
    if (dependencies.statistics === undefined)
      throw new ConflictError('STATISTICS_UNAVAILABLE', 'Statistics are not configured.');
    const query = new URL(request.url).searchParams;
    const scope = query.get('scope') ?? 'ALL';
    if (scope !== 'ALL' && scope !== 'MINE' && scope !== 'PENDING')
      throw new ApiValidationError('scope must be ALL, MINE, or PENDING.');
    const limit = readHistoryLimit(request);
    const cursor = readActivityCursor(query.get('cursor'));
    const wallets = await dependencies.access.controlledWallets(gameId, actor.id);
    return success(
      scope === 'PENDING'
        ? await dependencies.statistics.pending(
            gameId,
            wallets.map((wallet) => wallet.playerId),
            cursor,
            limit,
          )
        : await dependencies.statistics.activity(
            gameId,
            scope,
            wallets.map((wallet) => wallet.playerId),
            cursor,
  ...
  if (favoriteMatch !== null && request.method === 'POST') {
    ...
    if (typeof body.amount !== 'number' || !Number.isSafeInteger(body.amount) || body.amount <= 0)
      throw new ApiValidationError('amount must be a positive integer.');
    const gameId = parseResourceId(favoriteMatch[1], 'gameId');
    await dependencies.access.requireMember(gameId, actor.id);
    return success(await dependencies.games.toggleFavoriteAmount(gameId, body.amount));
  }
  ...
  if (bankruptcyMatch !== null && request.method === 'POST') {
    ...
    await dependencies.access.requirePlayerController(
      gameId,
      actor.id,
      controlledPlayerIdForBankingCommand(body),
    );
    if (dependencies.live !== undefined)
      return dependencies.live.mutate(gameId, actor, {
  ...
  if (transactionMatch !== null) {
    if (request.method === 'POST') {
      const gameId = parseResourceId(transactionMatch[1], 'gameId');
      await dependencies.access.requireMember(gameId, actor.id);
    ...
    if (request.method === 'GET') {
      ...
      await dependencies.access.requireMember(gameId, actor.id);
      return success(
        await dependencies.banking.listTransactions(gameId, readHistoryLimit(request)),
      );
    }
  }
  ...
  if (paymentRequestsMatch !== null && request.method === 'GET') {
    ...
    const wallets = await dependencies.access.controlledWallets(gameId, actor.id);
    return success(
      await dependencies.banking.listPaymentRequests(
        gameId,
        wallets.map((wallet) => wallet.playerId),
      ),
    );
  ...
  if (paymentRequestActionMatch !== null && request.method === 'POST') {
    ...
    if (paymentRequest === null)
      throw new NotFoundError('PAYMENT_REQUEST_NOT_FOUND', 'Payment request not found.');
    const controlledPlayerId =
      action === 'cancel' ? paymentRequest.creatorPlayerId : paymentRequest.approverPlayerId;
  ...
  if (playerTransactionMatch !== null && request.method === 'GET') {
    const gameId = parseResourceId(playerTransactionMatch[1], 'gameId');
    await dependencies.access.requireMember(gameId, actor.id);
    const playerId = parseResourceId(playerTransactionMatch[2], 'playerId');
    await dependencies.access.requirePlayerController(gameId, actor.id, playerId);
    return success(
...
function readCommandId(request: Request): string {
  ...
  if (
    commandId === null ||
    !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(commandId)
  )
    throw new ApiValidationError('x-command-id must be a UUID v4.');
  return commandId;
}
...
function readActivityCursor(value: string | null): { createdAt: string; id: string } | null {
  ...
  try {
    ...
    if (
      ...
      !/^[0-9a-f-]{36}$/iu.test(String((parsed as { id?: unknown }).id))
    )
      throw new Error();
    return {
...
function success<T>(data: T, status = 200, cookie?: string): Response {
  return Response.json({ data } satisfies ApiSuccess<T>, {
    status,
    ...(cookie === undefined ? {} : { headers: { 'set-cookie': cookie } }),
  });
}

async function enforceRateLimit(
  request: Request,
  repository: SecurityRateLimitRepository | undefined,
  requestId: string,
): Promise<void> {
    ...
    pathname === '/api/auth/login' || pathname === '/api/auth/register'
      ...
      : pathname === '/api/auth/password-reset' || pathname === '/api/auth/verification-email'
        ...
        : pathname === '/api/games/join/guest'
          ? { category: 'guest-join', limit: 12, seconds: 600 }
          : /^\/api\/games\/[^/]+\/invitations$/.test(pathname) && request.method === 'POST'
...
function readRequestId(request: Request): string {
  ...
  return candidate !== null && /^[A-Za-z0-9][A-Za-z0-9._-]{7,127}$/u.test(candidate)
    ? candidate
    : crypto.randomUUID();
}
...
function nextPlayerColor(players: readonly { color: string }[]): string {
  ...
  return (
    colors.find((color) => !players.some((player) => player.color === color)) ??
    colors[players.length % colors.length]
  );
}

async function joinCodeAccess(
  access: GameAccessService,
  joinCode: string,
  gameAccessPassword: string | undefined,
): Promise<{ gameId: string; status: string } | null> {
  const credentials = await access.getJoinCredentials(joinCode);
  if (credentials === null || !(await access.verifyGamePassword(gameAccessPassword, credentials)))
    return null;
