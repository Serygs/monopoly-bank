...
describe('API router', () => {
  ...
  it('uses the same generic response for known and unknown password-reset requests', async () => {
    ...
    const known = await router(
      jsonRequest('POST', '/api/auth/password-reset', { email: 'known@example.test' }),
    );
    const unknown = await router(
      jsonRequest('POST', '/api/auth/password-reset', { email: 'unknown@example.test' }),
    );
  ...
  it.each([
    ...
    const response = await createTestRouter()(
      jsonRequest('POST', '/api/games', {
        name: 'Friday Monopoly',
        startingBalance: 1500,
        passGoReward: 200,
  ...
  it('accepts a valid payment request', async () => {
    ...
    const response = await router(
      jsonRequest('POST', `/api/games/${gameId}/transactions`, {
  ...
  it.each([
    ...
    const response = await router(
      jsonRequest('POST', `/api/games/${gameId}/transactions`, { type, ...fields }),
    );
  ...
  it('does not expose an unowned legacy game through a normal authenticated API', async () => {
    const games: GameService = {
      ...
      createGameForOwner: async () => gameDetails,
      getGame: async () => gameDetails,
      deleteGame: async () => ({ gameId }),
      duplicateGame: async () => gameDetails,
      finishGame: async () => gameDetails,
      toggleFavoriteAmount: async () => [],
    };
  ...
  it('exposes an invitation code only to the game owner', async () => {
    const games: GameService = {
      ...
      getGame: async () => gameDetails,
      deleteGame: async () => ({ gameId }),
      duplicateGameForOwner: async () => gameDetails,
      finishGame: async () => gameDetails,
      toggleFavoriteAmount: async () => [],
    };
    const banking: BankingService = {
    ...
    await expect(response.json()).resolves.toMatchObject({
      data: { canManage: true, joinCode: 'TABLE42' },
    });
  });

  it('does not expose an invitation code to a player member', async () => {
    const response = await createTestRouter()(
      new Request(`https://example.test/api/games/${gameId}`),
    );

    await expect(response.json()).resolves.toEqual({
      data: { ...gameDetails, controlledWallets: [], controlledPlayerIds: [], canManage: false },
    });
  });
  ...
  it('creates a secure lobby invitation only through the owner route', async () => {
    ...
    const router = createApiRouter({
      ...
      access: {
        requireOwner: async (requestedGameId: string) => {
          invitationInput = {
    ...
    const response = await router(
      new Request(`https://example.test/api/games/${gameId}/invitations`, { method: 'POST' }),
    );
  ...
  it('does not distinguish expired or revoked invitation tokens from an invalid token', async () => {
    ...
    const response = await router(
      jsonRequest('POST', '/api/games/join', { invitationToken: 'A'.repeat(43) }),
    );
    expect(response.status).toBe(404);
  ...
  it('preserves the Durable Object WebSocket upgrade response', async () => {
    const upgrade = {
      status: 101,
      headers: new Headers({ 'x-request-id': 'live-request-id' }),
      body: null,
      statusText: '',
    } as Response;
    const router = createApiRouter({
    ...
    await expect(
      router(
        new Request(`https://example.test/api/games/${gameId}/live`, {
  ...
  it('returns 404 for a missing player history', async () => {
    ...
    const response = await router(
      new Request(`https://example.test/api/games/${gameId}/players/${firstPlayerId}/transactions`),
    );
    expect(response.status).toBe(404);
  });
  ...
  it('returns game transaction history', async () => {
    ...
    const response = await router(
      new Request(`https://example.test/api/games/${gameId}/transactions`),
    );
    expect(response.status).toBe(200);
  ...
  it('rejects malformed resource IDs before calling a service', async () => {
    const response = await createTestRouter()(
      new Request('https://example.test/api/games/not-a-uuid'),
    );
...
function createTestRouter(overrides: Partial<TestServiceOverrides> = {}) {
  ...
  const access = {
    ...
    requirePlayerController: async () => undefined,
    controlledWallets: async () => [],
  };
  ...
  return createApiRouter({
    ...
    profileStatistics: profileStatistics as never,
  });
}
