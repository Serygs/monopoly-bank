...
export async function parseGuestJoinGameRequest(request: Request): Promise<GuestJoinGameRequest> { const body = await parseJsonObject(request); const credentials = parseJoinGameBody(body); return { nickname: readNickname(body), avatar: readAvatar(body), ...credentials }; }
...
function parseJoinGameBody(body: Record<string, unknown>): JoinGameRequest { if (body.invitationToken !== undefined) return { invitationToken: readInvitationToken(body) }; return { joinCode: readJoinCode(body), ...(body.gameAccessPassword === undefined || body.gameAccessPassword === '' ? {} : { gameAccessPassword: readGamePassword(body, 'gameAccessPassword') }) }; }
