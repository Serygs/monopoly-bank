...
describe('D1 game statistics', () => {
  it('aggregates a fixture larger than 1000 ledger rows without a history limit', async () => {
    ...
    const summary = await new D1GameStatisticsRepository(
      database as unknown as D1Database,
    ).calculate(game, players);
  ...
  it('does not bind wallet IDs when loading all game activity', async () => {
    ...
    await new D1GameStatisticsRepository(database as unknown as D1Database).activity(
      'game',
      'ALL',
      ['wallet-a', 'wallet-b'],
      null,
