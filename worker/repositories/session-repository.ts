export interface SessionRepository { create(id: string, userId: string, tokenHash: string, expiresAt: string): Promise<void>; findUserId(tokenHash: string): Promise<string | null>; delete(tokenHash: string): Promise<void>; deleteAllForUser(userId: string): Promise<void>; deleteExpired(): Promise<void>; }
export class D1SessionRepository implements SessionRepository {
  ...
  async deleteExpired(): Promise<void> { await this.database.prepare("DELETE FROM user_sessions WHERE expires_at <= strftime('%Y-%m-%dT%H:%M:%fZ', 'now')").run(); }
}
