export interface SessionRepository { create(id: string, userId: string, tokenHash: string, expiresAt: string): Promise<void>; findUserId(tokenHash: string): Promise<string | null>; delete(tokenHash: string): Promise<void>; }
export class D1SessionRepository implements SessionRepository {
  ...
  async findUserId(tokenHash: string): Promise<string | null> { const row = await this.database.prepare("SELECT user_id FROM user_sessions WHERE token_hash = ? AND expires_at > CURRENT_TIMESTAMP").bind(tokenHash).first<{ user_id: string }>(); return row?.user_id ?? null; }
