...
describe('D1GameAccessRepository controller access', () => {
  it('reads only the actor controls in primary/local order', async () => {
    const database = new ControllerDatabase([
      { player_id: 'primary', controller_kind: 'PRIMARY' },
      { player_id: 'local', controller_kind: 'LOCAL' },
    ]);
    const repository = new D1GameAccessRepository(database as unknown as D1Database);

    await expect(repository.listPlayerControllers('game', 'actor')).resolves.toEqual([
      { playerId: 'primary', kind: 'PRIMARY' },
      { playerId: 'local', kind: 'LOCAL' },
    ]);
...
class ControllerDatabase {
  ...
  constructor(
    private readonly rows: Array<{ player_id: string; controller_kind: 'PRIMARY' | 'LOCAL' }>,
  ) {}
  prepare(): {
    bind: (...values: unknown[]) => {
      all: () => Promise<{
        results: Array<{ player_id: string; controller_kind: 'PRIMARY' | 'LOCAL' }>;
      }>;
    ...
    return {
      bind: (...values) => {
        ...
        return {
          ...
          first: async <T>() => this.firstResult as T | null,
        };
      },
    };
  }
