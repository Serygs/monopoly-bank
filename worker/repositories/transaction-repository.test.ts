...
describe('D1TransactionRepository', () => {
  it('writes a transaction and all participant records in one D1 batch', async () => {
    ...
    expect(database.batches[0].slice(1).map((statement) => statement.values)).toEqual([
      ['transaction-1', 'game-1', 'payer', -100],
      ['transaction-1', 'game-1', 'recipient-1', 50],
      ['transaction-1', 'game-1', 'recipient-2', 50],
    ]);
  });
  ...
  it('returns a zero-balance bankruptcy transaction with no balance participants', async () => {
    ...
    const transaction = await new D1TransactionRepository(
      database as unknown as D1Database,
    ).getById('game', 'transaction');
