...
export class D1AuthTokenRepository implements AuthTokenRepository {
  ...
  async issue(input: { id: string; userId: string; purpose: AuthTokenPurpose; tokenHash: string; expiresAt: string }): Promise<boolean> {
    try {
      const result = await this.database.batch([
        this.database.prepare("UPDATE auth_email_tokens SET consumed_at = CURRENT_TIMESTAMP WHERE user_id = ? AND purpose = ? AND consumed_at IS NULL AND (expires_at <= strftime('%Y-%m-%dT%H:%M:%fZ', 'now') OR created_at <= datetime('now', '-10 minutes'))").bind(input.userId, input.purpose),
