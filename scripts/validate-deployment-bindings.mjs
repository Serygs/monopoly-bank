...
if (!configPath || !expectedDatabaseId || !expectedDatabaseName || !expectedWorkerName)
  throw new Error(
    'WRANGLER_CONFIG_PATH, EXPECTED_D1_DATABASE_ID, EXPECTED_D1_DATABASE_NAME, and EXPECTED_WORKER_NAME are required.',
  );
...
if (configuration.name !== expectedWorkerName)
  throw new Error('Worker name does not match the approved deployment target.');
if (
  databases.length !== 1 ||
  !database ||
  ...
  throw new Error('D1 binding does not match the approved deployment target.');
if (
  !configuration.durable_objects?.bindings?.some(
    (binding) => binding.name === 'GAME_SESSIONS' && binding.class_name === 'GameSession',
  )
)
  throw new Error('GAME_SESSIONS Durable Object binding is missing.');
