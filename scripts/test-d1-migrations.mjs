...
try {
  ...
  const schema = await sql("SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('player_controllers', 'payment_requests', 'command_ledger', 'security_rate_limits', 'game_final_snapshots') ORDER BY name");
  for (const table of ['player_controllers', 'payment_requests', 'command_ledger', 'security_rate_limits', 'game_final_snapshots']) {
  ...
  for (const migration of await readdir('migrations')) {
    if (migration <= '0015_activity_statistics_and_final_snapshots.sql') await cp(join('migrations', migration), join(legacyMigrations, migration));
  }
  ...
  await legacySql("INSERT INTO users (id, nickname, avatar, password_hash, password_salt, account_type) VALUES ('user-upgrade-1', 'Existing player', '😀', 'fixture-password-hash', 'fixture-password-salt', 'REGISTERED'); INSERT INTO games (id, name, starting_balance, pass_go_reward, currency, status, payment_mode) VALUES ('game-upgrade-1', 'Existing game', 1500, 200, 'K', 'ACTIVE', 'FAST'); INSERT INTO players (id, game_id, name, color, balance, user_id) VALUES ('player-upgrade-1', 'game-upgrade-1', 'Existing player', '#123456', 1500, 'user-upgrade-1');");
  await command(['d1', 'migrations', 'apply', 'MONOPOLY_BANK_DB', '--local', '--persist-to', legacyPersistence]);
  const upgradedFixture = await legacySql("SELECT p.id AS player_id, u.id AS user_id FROM players p JOIN users u ON u.id = p.user_id WHERE p.id = 'player-upgrade-1'");
