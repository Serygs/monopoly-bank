...
if (!home.ok || !home.headers.get('content-type')?.includes('text/html'))
  throw new Error(`Application shell smoke test failed with ${home.status}.`);
const profile = await fetch(new URL('/api/profile', base), { redirect: 'manual' });
if (profile.status !== 401)
  throw new Error(
    `Authenticated API boundary smoke test expected 401, received ${profile.status}.`,
  );
for (const header of [
  ...
  if (!profile.headers.has(header))
    throw new Error(`Authenticated API response is missing ${header}.`);
}
