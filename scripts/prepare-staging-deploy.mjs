...
if (!databaseId || !databaseIdPattern.test(databaseId)) throw new Error('CLOUDFLARE_STAGING_D1_DATABASE_ID must be a D1 database UUID.');
if (!databaseName) throw new Error('CLOUDFLARE_STAGING_D1_DATABASE_NAME is required.');
if (!/^[a-z0-9-]+$/.test(workerName) || workerName === 'monopoly-bank') throw new Error('CLOUDFLARE_STAGING_WORKER_NAME must be a non-production Worker name.');
...
if (!database || databases.length !== 1) throw new Error('The build configuration must contain exactly the MONOPOLY_BANK_DB binding.');
